...
class App:
	def __init__(self, window, window_title, video_source=0):
		...
		self.scale = 100
		self.window = window
		self.window.geometry("500x130+300+300")
		...
		self.label1 = Label(self.window,text="1. Browse and select The Folder to your Project folder where settings.py is present").place(x=10,y=30)
		self.label2 = Label(self.window,text="2.Press Deploy").place(x=10,y=50)
		self.folderbutton = Button(self.window,text="Browse Folder",width=12,height=1,command=self.get_dirname).place(x=10,y=70)
		self.folderSuccess = StringVar()
		self.folderError = StringVar()

		self.folderLabelSuccess = Label(self.window, fg='green', textvariable=self.folderSuccess).place(x=150,y=70)
		self.folderLabelError 	= Label(self.window, fg='red', textvariable=self.folderError).place(x=150,y=70)
		self.deploybutton = Button(self.window,text="Deploy",width=12,height=1,command=self.deploy).place(x=190,y=100)
	...
	def deploy(self):
		if self.directoryFlag == 0:
			...
			deploy.Django_Heroku_Deploy(self.folderSuccess.get())
